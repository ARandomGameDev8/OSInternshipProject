...
"""
master.py — Native Windows ETW Call Tracer with 6-subplot live GUI 
...
class NativeETWConsumer:
    def __init__(self, target_pids, event_callback):
        ...
        # Keep C callback reference to avoid GC collection
    ...
    def _internal_callback(self, record_ptr):
        ...
        if pid in self.target_pids:
            ...
            # HeapAlloc call event (Id 1 or 33)
            ...
            # HeapFree call event (Id 2 or 34)
    ...
    def start(self):
        ...
        props.Wnode_Flags = 0x00020000  # WNODE_FLAG_TRACED_GUID
        ...
        # Stop existing session with same name if leftover
        ...
        # Enable Microsoft-Windows-Heap provider
        self._advapi32.EnableTraceEx2(
            sess_handle,
            ctypes.byref(heap_guid),
            1,  # CONTROL_CODE_ENABLE_PROVIDER
        ...
        logfile.LogFileMode = 0x00000100 | 0x10000000  # REAL_TIME | PROCESS_TRACE_MODE_EVENT_RECORD
...
class ProcessRAM_Graph:
    """
    Stores call event streams:
    - mallocSize: PID -> malloc call allocation size vs time
    - freeSize:   PID -> free call deallocation size vs time
    """
    ...
    def __init__(self, pid: int, name: str):
        ...
        self.mallocSize  = []  # size allocated in GB per call
        self.freeSize    = []  # size deallocated in GB per call
    ...
    def compute_kinematics(series, time_points):
        """Computes velocity (GB/s) and acceleration (GB/s²) for call streams."""
        if len(series) < 2:
...
class Monitor:
    ...
    def __init__(self, filt):
        ...
        # Initialize Native Windows ETW Call Consumer
    ...
    def _handle_heap_call_event(self, pid: int, event_type: str, size_bytes: int):
        """Processes real-time HeapAlloc / HeapFree ETW calls."""
        now_ns = time.time_ns()
            ...
            if force_redraw:
                ...
                for i, pid in enumerate(pids):
                    ...
                    if graph and graph.has_data():
                        ...
                        # Kinematics calculations on explicit call sizes
